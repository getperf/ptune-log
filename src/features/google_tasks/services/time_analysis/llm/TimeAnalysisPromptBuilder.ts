...
- 出力時のタスク名は、次の形式に必ず変換すること。
  - 子タスク（parentTaskKeyあり）: 「親タスクの子タスク：子タスク名」
    - 子タスクは省略しない（短縮禁止）。
    - 例: 「時間分析結合の子タスク：プロトタイプ」
  - 親タスク（parentTaskKeyなし）: タスク名そのまま。長い場合のみ末尾を「…」で省略してよい。
- 入力に「親__子」のような連結が含まれていても、出力は必ず上記形式にすること。
